...
# Adjustments for Consors Bank: Klaus Kneupner
# Date: Feb 2020
# Converts a CSV file from Consors Bank for import to YNAB (online). It includes a conversion from EUR to DKK (factor 7.45)
#
# Reads a CSV file from Consors and outputs
#  a QIF file for YNAB.
...
class Transaction_DK(object):
    def __init__(self, Date, Payee, Category='', Memo='', amount_str=0.0, Cleared=True):
        ...
        self.Memo = Memo + ' (EUR' + amount_str + ')'
        self.Cleared = Cleared
        amount = float(amount_str.replace('.','', 1).replace(',','.', 1))
        self.flow = round(amount * 7.45 , 2)
        if (self.flow < 0.0):
...
def reader(line, verbose=0, lineno=0):
    ...
    text = line[2]
    memo = line[6]
    if not ('Buchung' in line[0]): 
        t = Transaction_DK(line[0], text, amount_str=line[7], Memo=memo, Cleared=True)
        return t
    return None
...
def main(inp, outp, as_qif=False, verbose=0, qifopt=None):
    ...
    with codecs.open(inp,  encoding='utf-8') as fin:
